    ...
    for (let j = 0 ; j < listaDeIDs.length ; j++){
        const quizzesDoUsuario = axios.get(`https://mock-api.driven.com.br/api/v6/buzzquizz/quizzes/${listaDeIDs[j]}`).then(response=>{const image = response.data.image;
            ...
            lista.innerHTML += `
                <li class="quizz">
                    <img src=${image} alt="">
                    <div class="quizz-configuracao"><ion-icon class="edit" name="create-outline" onclick="carregarEdicao(${listaDeIDs[j]})"></ion-icon><ion-icon class="trash" name="trash-outline" onclick="deleteQuiz('${listaDeIDs[j]} ', '${listaDeKeys[j]}','${j}'></ion-icon></div>
